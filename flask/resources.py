...
class FavoriteRecipes(Resource):
    ...
    def put(self, username, **kwargs):
        ...
        try:
            fav: Favorite = Favorite(username=username,
                                     recipe_id=kwargs[RECIPE_ID])
            db.session.add(fav)
            db.session.commit()
        except:
            db.session.rollback()
            return {
                "message": "could not find recipe to add",
                "recipe_id": kwargs[RECIPE_ID]
            }, 404
...
class OwnRecipes(Resource):
    ...
    def put(self, username):
        ...
        if RECIPE_ID in json_data:
            ...
            current_user = get_jwt_identity()
            user: User = db.session.get(User, current_user)
            recipe: Recipe = db.session.get(Recipe, json_data[RECIPE_ID])

            if not recipe:
                return {"message": "Recipe not found"}, 404

            if (user.username != username or
                (recipe.username != current_user and not user.is_admin)):
                return {
                    "message": "You do not have permission to Edit this"
                }, 401
            if PICTURE in json_data:
                db.session.execute(update(Recipe), [{
    ...
    def delete(self, username, **kwargs):
        ...
        current_user = get_jwt_identity()
        user: User = db.session.get(User, current_user)
        recipe: Recipe = db.session.get(Recipe, kwargs[RECIPE_ID])
...
class RateRecipe(Resource):
    ...
    def get(self, **kwargs):
        ...
        with db.engine.connect() as conn:
            result = conn.execute(
                text(
                    "SELECT COUNT(*) from review WHERE recipe_id = :id AND rating = 1"
                ), {
                    "id": kwargs[RECIPE_ID]
                }).scalar_one()
            return result
    ...
    def put(self, **kwargs):
        ...
        """
        review: Review = db.session.get(Review, {
        ...
        if review:
            ...
            review = Review(username=kwargs[USERNAME],
                            recipe_id=kwargs[RECIPE_ID],
                            rating=kwargs[RECIPE_ID])
